...
public class SanPhamDAO  implements ISanPhamDAO {
	...
	public List<SanPham> findAll(int begin, int quantity, String typeSort, String sortBy) {
		...
		System.out.println(" or der :"+begin);
		try {
			if (begin == -1) {
				list = session.createQuery("from sanpham sp " + order).getResultList();
			} else {
				list = session.createQuery("from sanpham sp " + order).setFirstResult(begin).setMaxResults(quantity).getResultList();
			}
			return list;
		} catch (Exception e) {
